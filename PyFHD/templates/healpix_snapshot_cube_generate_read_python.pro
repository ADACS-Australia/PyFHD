PRO healpix_snapshot_cube_generate_read_python,obs_in,status_str,psf_in,cal,params,vis_arr,vis_model_arr=vis_model_arr,$
    ...
    obs_out=obs_out,psf_out=psf_out,ps_tile_flag_list=ps_tile_flag_list,python_grid_path=python_grid_path,_Extra=extra

  print,"START OF healpix_snapshot_cube_generate ",python_grid_path
  ...
  IF Keyword_Set(split_ps_export) THEN BEGIN
    ...
    ; if keyword_set(save_imagecube) then imagecube_filepath = "/fred/oz048/jline/ADACS/test_PyFHD/grid_real_data/pyfhd_nb_data_gd_woden_redo_redo/fhd_pyfhd_nb_data_gd_woden_redo_redo/Healpix/"+obs_in.obsname+['_even','_odd'] + '_gridded_imagecube.sav'
  ENDIF ELSE BEGIN
    n_iter=1
  ...
  ; TODO get rid of this hard-coding please god
  dirty_flag = 1
  model_flag = 1
  ...
  FOR iter=0,n_iter-1 DO BEGIN
    ...
    residual_arr1=vis_model_freq_split_read_python(obs_in,status_str,psf_in,params,vis_weights_use,obs_out=obs,psf_out=psf,rephase_weights=rephase_weights,$
      ...
      save_uvf=save_uvf, uvf_name=uvf_name[iter],bi_use=*bi_use[iter],python_grid_path=python_grid_path,_Extra=extra)
