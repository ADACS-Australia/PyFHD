FUNCTION vis_model_freq_split_read_python,obs,status_str,psf,params,vis_weights,model_uv_arr=model_uv_arr,vis_data_arr=vis_data_arr,vis_model_arr=vis_model_arr,$
    ...
    obs_out=obs_out,psf_out=psf_out,save_uvf=save_uvf, uvf_name=uvf_name,bi_use=bi_use,python_grid_path=python_grid_path,_Extra=extra

  print,"START OF vis_model_freq_split_read_python ",python_grid_path
  model_flag = extra.model_flag
  ; save_uvf=1
  ...
  FOR pol_i=0,n_pol-1 DO BEGIN
    ...
    FOR fi=0L,nf-1 DO BEGIN
      ...
      ; n_vis = nf_vis[fi]
        ...
        ; Open the dataset that contains the frequency slice we want
        dataset_dirty_uv = H5D_OPEN(file_id, 'dirty_uv' + sub)
        dataset_weights_holo = H5D_OPEN(file_id, 'weights_holo' + sub)
        dataset_variance_holo = H5D_OPEN(file_id, 'variance_holo' + sub)
        dataset_model_return = H5D_OPEN(file_id, 'model_return' + sub)
        dataset_n_vis = H5D_OPEN(file_id, 'n_vis' + sub)

        ; Read in the actual image data.
        dirty_uv_struct = H5D_READ(dataset_dirty_uv)
        weights_holo_struct = H5D_READ(dataset_weights_holo)
        ...
        weights_holo = weights_holo_struct.r + Complex(0,1)*weights_holo_struct.i
      ...
      ; release the hdf5 datasets
      H5D_CLOSE, dataset_dirty_uv
      H5D_CLOSE, dataset_weights_holo
      H5D_CLOSE, dataset_variance_holo
      H5D_CLOSE, dataset_model_return
      H5D_CLOSE, dataset_n_vis
