...
def pyfhd_parser():
    ...
    calibration.add_argument('--max-cal-baseline', type = float, default = None, help = 'The maximum baseline length in wavelengths to be used in calibration. If max_baseline is smaller, it will be used instead.')
    calibration.add_argument('--allow-sidelobe-cal-sources', default = False, action = 'store_true', help = 'Allows PyFHD to calibrate on sources in the sidelobes.\nForces the beam_threshold to 0.01 in order to go down to 1%% of the beam to capture sidelobe sources during the generation of a calibration source catalog for the particular observation.')
    calibration.add_argument('--cable-bandpass-fit', default = False, action = 'store_true', help = 'Average the calibration solutions across tiles within a cable grouping for the particular instrument.\nDependency: instrument_config/<instrument>_cable_length.txt')
    ...
    calibration.add_argument('--calibration-polyfit', default = False, action = 'store_true', help = 'Calculates a polynomial fit across the frequency band for the gain, and allows a cable reflection to be fit.\nThe orders of the polynomial fit are determined by cal_phase_degree_fit and cal_amp_degree_fit.\nIf unset, no polynomial fit or cable reflection fit are used.')
    ...
    calibration.add_argument('--cal-reflection-hyperresolve', default = False, action = 'store_true', help = 'Hyperresolve and fit residual gains using nominal reflection modes (calculated from cal_reflection_mode_delay or cal_reflection_mode_theory),\nproducing a finetuned mode fit, amplitude, and phase.\nWill be ignored if cal_reflection_mode_file is set because it is assumed that a file read-in contains mode/amp/phase to use.')
    ...
    calibration.add_argument('--cal-time-average', default=False, action='store_true', help="Performs a time average of the model/data visibilities over the time steps in the observation to reduce the number of equations that are used in the linear-least squares solver. This improves computation time, but will downweight longer baseline visibilities due to their faster phase variation.")
    calibration.add_argument('--auto-ratio-calibration', default=False, action='store_true', help= 'Calculates the auto ratios for cable reflections and enables global bandpass')
    ...
    flag.add_argument('-fcf', '--flag-calibration-frequencies', default = False, action = 'store_true', help = 'If True, flags frequencies based off 0 calibration gain, if False, ignores the calibration gain for frequencies')
    ...
    deconv.add_argument('--filter-background', default = False, action = 'store_true', help = 'Filters out large-scale background fluctuations before deconvolving point sources.')
    ...
    sim.add_argument('--extra-vis-filepath', type = Path, default = None, help = 'Optionally add general visibilities to the simulation, must be a uvfits file.')
    ...
    healpix.add_argument('--ps-tile-flag-list', type = list, default = [], action = 'append', help = "A list of tile names to manually flag in the healpix export. I repeat, a list of tile names, NOT tile indices.")
