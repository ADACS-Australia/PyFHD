...
def before_file(tag, run, data_dir, model_dir):
    ...
    h5_save_dict["pyfhd_config"] = {
        ...
        "model_file_path" : str(Path(model_dir, f"{tag}.uvfits")) if tag == "point_zenith" else str(model_dir),
        "obs_id": tag,
        "instrument": "mwa",
        "n_pol": 2
    }
...
def after_file(tag, run, data_dir):
    ...
    vis_model_arr = np.empty([sav_dict['vis_model_arr'].shape[0], sav_dict['vis_model_arr'][0].shape[1], sav_dict['vis_model_arr'][0].shape[0]], dtype = np.complex128)
