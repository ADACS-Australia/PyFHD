...
def before_file(tag, run, subfunc, data_dir):
    if ([tag, subfunc, run] in skip_tests):
        return None
    before_file = Path(data_dir, f"{tag}_{run}{f'_{subfunc}_' if subfunc != '' else ''}_before_{data_dir.name}.h5")
...
def after_file(tag, run, subfunc, data_dir):
    if ([tag, subfunc, run] in skip_tests):
        return None
    after_file = Path(data_dir, f"{tag}_{run}{f'_{subfunc}_' if subfunc != '' else ''}_after_{data_dir.name}.h5")
    # If the h5 file already exists and has been created, return the path to it
