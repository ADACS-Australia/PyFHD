...
def before_file(tag, run, data_dir):
    ...
    pyfhd_config["model_file_path"] = str(
        Path(data_dir, "point_zenith_8s_80kHz_analy_autos.uvfits")
    )
    ...
    subroutine_file = Path(
        subroutine_before, f"{tag}_{run}_before_{subroutine_before.name}.h5"
    )
    ...
    vis_model_arr, params_model = vis_model_transfer(
        pyfhd_config, h5_save_dict["obs"], Logger(1)
    )
...
def after_file(tag, run, data_dir):
    ...
    expected_mode_params = np.empty(
        (h5_save_dict["cal"]["n_pol"], h5_save_dict["obs"]["n_tile"], 3)
    )
    ...
    for pol_i in range(h5_save_dict["cal"]["n_pol"]):
        for tile_i in range(h5_save_dict["obs"]["n_tile"]):
            expected_amp_params[pol_i, tile_i] = np.transpose(
                h5_save_dict["cal"]["amp_params"][tile_i, pol_i]
            )
            expected_phase_params[pol_i, tile_i] = np.vstack(
                h5_save_dict["cal"]["phase_params"][tile_i, pol_i]
            ).flatten()
            if h5_save_dict["cal"]["phase_params"][tile_i, pol_i][0] is None:
                ...
                expected_mode_params[pol_i, tile_i] = h5_save_dict["cal"][
                    "mode_params"
                ][tile_i, pol_i]
...
def test_calibrate(before_file, after_file):
    ...
    assert_allclose(
        cal["gain"][actual_nan], h5_after["cal"]["gain"][actual_nan], atol=1.2e-6
    )
