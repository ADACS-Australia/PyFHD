...
def convert_before_sav_to_npz(test_dir):
    """Takes the before .sav file out of FHD function `calculate_adaptive_gain`
    and converts into a numpy format"""

    sav_dict = convert_sav_to_dict(f"{test_dir}/before_calculate_adaptive_gain.sav", "meh")
    
    np.savez(f"{test_dir}/before_calculate_adaptive_gain.npz",
...
def convert_after_sav_to_npz(test_dir):
    """Takes the after .sav file out of FHD function `calculate_adaptive_gain`
    and converts into a numpy format"""

    sav_dict = convert_sav_to_dict(f"{test_dir}/after_calculate_adaptive_gain.sav", "meh")
    
    # np.savez(f"{test_dir}/after_calculate_adaptive_gain.npz",
    #             gain_list=sav_dict['gain_list'],
    #             gain=sav_dict['gain'])
...
def convert_sav_to_npz(test_dir):
    """Load the inputs and outputs needed for testing `calculate_adaptive_gain`"""
...
def run_test(data_loc):
    """Runs the test on `calculate_adaptive_gain` - reads in the data in `data_loc`,
    and then calls `calculate_adaptive_gain`, checking the outputs match expectations"""

    gain_list, convergence_list, iter, base_gain, final_convergence_estimate_in = get_data_npz(f"{data_loc}/before_calculate_adaptive_gain.npz")
    expected_gain_list, expected_gain, final_convergence_estimate_out = get_data_npz(f"{data_loc}/after_calculate_adaptive_gain.npz")
