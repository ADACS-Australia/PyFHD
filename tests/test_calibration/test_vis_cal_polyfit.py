...
def before_file(tag, run, data_dir):
    ...
    pyfhd_config["cable_reflection_coefficients"] = str(importlib_resources.files('PyFHD.templates').joinpath('mwa_cable_reflection_coefficients.txt'))
    pyfhd_config["cable_lengths"] = str(importlib_resources.files('PyFHD.templates').joinpath('mwa_cable_length.txt'))
...
def after_file(tag, run, data_dir):
    ...
    for pol_i in range(cal_return['n_pol']):
        for tile_i in range(obs['n_tile']):
            expected_amp_params[pol_i, tile_i] = np.transpose(cal_return['amp_params'][tile_i, pol_i])
            expected_phase_params[pol_i, tile_i] = np.vstack(cal_return['phase_params'][tile_i, pol_i]).flatten()
            if cal_return['phase_params'][tile_i, pol_i][0] is None:
                expected_mode_params[pol_i, tile_i] = np.full(3, np.nan, dtype = np.float64)
            else:
                expected_mode_params[pol_i, tile_i] = cal_return['mode_params'][tile_i, pol_i]
