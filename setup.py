...
class GitInfo(setuptools.Command):
    ...
    description = (
        'Create the file "PyFHD/PyFHD_gitinfo.json" containing git information '
    )
    user_options = []
    ...
    def finalize_options(self):
        ...
        setuptools.Command to work)"""
        if self.git_info:
            print("Creating file PyFHD/PyFHD_gitinfo.txt")
    ...
    def run(self):
        ...
        save_path = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "PyFHD", "PyFHD_gitinfo.txt"
        )

        with open(save_path, "w") as outfile:
            outfile.write(
                get_commandline_output(["git", "describe", "--always"]) + "\n"
            )
            outfile.write(
                get_commandline_output(["git", "log", "-1", "--format=%cd"]) + "\n"
            )
            outfile.write(
                get_commandline_output(["git", "rev-parse", "--abbrev-ref", "HEAD"])
                + "\n"
            )
